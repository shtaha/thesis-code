...
from ..data_utils import indices_to_hot, is_nonetype, extract_history_windows
...
def obses_to_lgraphs(obses, dones, case, mask_targets=None, n_window=1, max_length=-1):
    ...
    for i, (obs, done) in enumerate(zip(obses, dones)):
        ...
        if mask_history[i]:
            ...
            queue.append(graph_dict)

            if mask_targets[i]:
                assert len(queue) == n_window + 1

                graph_dict = dict()
                graph_dict["senders"] = queue[-1]["senders"]
                graph_dict["receivers"] = queue[-1]["receivers"]

                graph_dict["globals"] = np.hstack([g_d["globals"] for g_d in queue])
                graph_dict["nodes"] = np.hstack([g_d["nodes"] for g_d in queue])
                graph_dict["edges"] = np.hstack([g_d["edges"] for g_d in queue])

                lgraphs.append(graph_dict)

        if 0 < max_length == len(lgraphs):
            break

    assert len(lgraphs) == mask_targets.sum()
    ...
    #
    # else:
    #     lgraphs = single_graphs_dict_list

    return lgraphs


def obses_to_cgraphs(obses, dones, case, mask=None, n_window=1, max_length=-1):
    graphs_dict_list = obses_to_lgraphs(
        obses, dones, case, mask_targets=mask, n_window=n_window, max_length=max_length
    )
